...
pub struct CronScheduler {
    ...
    storage: Option<Arc<xira_common::storage::SqliteStorage>>,
}
...
impl CronScheduler {
    ...
    pub fn with_storage(storage: Arc<xira_common::storage::SqliteStorage>) -> Self {
    ...
    fn persist_job(&self, job: &CronJob) {
        if let Some(ref storage) = self.storage {
            ...
            let _ = storage.execute_params(
                ...
                &[
                    &job.id as &dyn xira_common::rusqlite::types::ToSql,
                    &job.name,
                    &schedule_json,
                    &job.url,
                    &job.method,
                    ...
                    &job.last_status.map(|s| s as i64) as &dyn xira_common::rusqlite::types::ToSql,
    ...
    pub async fn remove_job(&self, id: &str) -> bool {
        if let Some(ref storage) = self.storage {
            let _ = storage.execute_params("DELETE FROM cron_jobs WHERE id = ?1", &[&id as &dyn xira_common::rusqlite::types::ToSql]);
        }
        let mut jobs = self.jobs.write().await;
        let len = jobs.len();
