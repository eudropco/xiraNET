...
pub async fn register_service(
    ...
    if req.name.is_empty() || req.prefix.is_empty() || req.upstream.is_empty() {
        return HttpResponse::BadRequest().json(ApiResponse::<()>::error(
            "name, prefix, and upstream are required",
        ));
    }
    ...
    HttpResponse::Created().json(ApiResponse::ok(
        format!("Service '{}' registered successfully", entry.name),
        entry,
    ))
}
...
pub async fn unregister_service(
    ...
    match registry.unregister(&id) {
        Some(entry) => HttpResponse::Ok().json(ApiResponse::ok(
...
pub async fn check_service_health(
    ...
    let service = match services.iter().find(|s| s.id == id) {
        Some(s) => s,
        None => {
            return HttpResponse::NotFound().json(ApiResponse::<()>::error("Service not found"))
        }
    };
        ...
        .await
    {
        Ok(resp) if resp.status().is_success() => {
            ...
            HttpResponse::Ok().json(ApiResponse::ok(
                format!("'{}' is UP", service.name),
                serde_json::json!({"name": service.name, "status": "UP"}),
            ))
        }
        Ok(resp) => {
        ...
        Err(e) => {
            registry.update_status(&id, ServiceStatus::Down);
            HttpResponse::Ok().json(ApiResponse::ok(
                format!("'{}' is DOWN: {}", service.name, e),
                serde_json::json!({"name": service.name, "status": "DOWN", "error": e.to_string()}),
            ))
        }
    }
}
...
pub async fn get_circuit_breakers(cb: web::Data<CircuitBreakerManager>) -> HttpResponse {
    ...
    HttpResponse::Ok().json(ApiResponse::ok(
        ...
        serde_json::json!({"breakers": report}),
    ))
}
